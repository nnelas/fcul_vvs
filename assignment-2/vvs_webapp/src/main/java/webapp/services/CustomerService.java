...
/**
 * Handles customer transactions. Each public method implements a transaction
 * script.
 * 
 * @author
 * @version
...
public enum CustomerService {
	...
	public void removeAddressesToCustomer(int customerVat) throws ApplicationException {
		if (!isValidVAT(customerVat))
			throw new ApplicationException("Invalid VAT number: " + customerVat);
		else
			try {
				List<AddressRowDataGateway> addressList = new AddressRowDataGateway().getCustomerAddresses(customerVat);
				for (AddressRowDataGateway addressRowDataGateway : addressList) {
					addressRowDataGateway.remove();
				}

			} catch (PersistenceException e) {
				throw new ApplicationException(
						"Can't remove the addresses of customer with vat number " + customerVat + ".", e);
			}
	}
