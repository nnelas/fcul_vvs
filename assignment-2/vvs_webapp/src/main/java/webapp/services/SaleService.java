...
/**
 * Handles sales transactions. Each public method implements a transaction
 * script.
 * 
 * @author
 * @version
...
public enum SaleService {
		...
		if (!isValidVAT(vat))
			...
			try {
				...
				for (SaleRowDataGateway sl : sales) {
					list.add(new SaleDTO(sl.getId(), sl.getData(), sl.getTotal(), sl.getStatusId(),
							sl.getCustomerVat()));
				}
	...
	public void addSale(int customerVat) throws ApplicationException {
		...
		try {
			new CustomerFinder().getCustomerByVATNumber(customerVat);

			SaleRowDataGateway sale = new SaleRowDataGateway(customerVat, new Date());
			sale.insert();

		} catch (PersistenceException e) {
			throw new ApplicationException("Can't add sale to customer with vat number " + customerVat + ".", e);
		}
	}
	...
	public void removeSalesOfCustomerVat(int vat) throws ApplicationException {
	...
	public void removeSalesDeliveryOfCustomerVat(int vat) throws ApplicationException {
